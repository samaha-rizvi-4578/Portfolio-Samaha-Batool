...
const Navbar = () => {
  ...
  return (
    <nav className={`w-full flex items-center bg-gradient-to-b from-black sm:bg-none p-8 sm:px-16 sm:py-10 fixed z-40 ${scrolled ? 'bg-opacity-90' : ''}`}>
              ...
              {navLinks.map((nav) => (
                <li
                  ...
                  className={`font-poppins font-medium cursor-pointer text-[16px] ${
                    active === nav.id ? "text-quaternary" : "text-primary"
                  }`}
                  onClick={() => {
                    setToggle(!toggle);
                    setActive(nav.id);
